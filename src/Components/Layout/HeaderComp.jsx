  ...
  return (
    <>
      ...
      >
        <div className="max-w-[1400px] mx-auto px-5 sm:px-8 py-4 flex items-center justify-between">
          ...
          <Link href="/" className="text-2xl font-black tracking-tight shrink-0">
            <span className="text-yellow-400">Black</span>
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm font-semibold tracking-wide uppercase">
            {navLinks.map(({ href, label, icon: Icon }) => (
              <Link
                key={href}
                href={href}
                className="nav-link-desktop flex items-center gap-1.5 text-white/60 hover:text-yellow-400 transition-colors duration-200"
              >
                <Icon size={14} />
                {label}
              </Link>
          ...
          <Link
            href="/pricing"
            className="hidden md:flex items-center gap-2 bg-yellow-400 text-black px-5 py-2 rounded-full font-bold hover:scale-105 hover:bg-yellow-300 transition-all duration-200 shrink-0"
          >
            <FiTag size={14} />
            Get Pricing
          </Link>
          ...
          <button
            className="md:hidden flex flex-col justify-center items-center gap-[6px] w-10 h-10 rounded-lg hover:bg-white/10 transition"
            onClick={() => setMenuOpen((v) => !v)}
            aria-label={menuOpen ? "Close menu" : "Open menu"}
          >
            <span className={`ham-line ham-line-top ${menuOpen ? "open" : ""}`} />
            <span className={`ham-line ham-line-mid ${menuOpen ? "open" : ""}`} />
            <span className={`ham-line ham-line-bot ${menuOpen ? "open" : ""}`} />
          </button>
        </div>
      </header>
      ...
      >
        {/* Close button */}
        <button
          className="absolute top-5 right-5 w-9 h-9 flex items-center justify-center rounded-lg text-white/50 hover:text-yellow-400 hover:bg-white/10 transition"
          onClick={() => setMenuOpen(false)}
        ...
        <nav className="flex flex-col gap-1">
          {navLinks.map(({ href, label, icon: Icon }) => (
            <Link
              ...
              href={href}
              onClick={() => setMenuOpen(false)}
              className="nav-item flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold text-white/60 hover:text-yellow-400 hover:bg-yellow-400/8 transition-all duration-200"
            >
              <Icon size={17} />
              {label}
            </Link>
        ...
        <Link
          href="/pricing"
          onClick={() => setMenuOpen(false)}
          className="nav-item nav-cta mt-8 flex items-center justify-center gap-2 bg-yellow-400 text-black px-5 py-3 rounded-full font-bold hover:bg-yellow-300 transition-all duration-200"
        >
          <FiTag size={15} />
          Get Pricing
        </Link>
        ...
        <p className="mt-auto text-[11px] text-white/20 text-center">
          © 2026 BlackGiraffe. All rights reserved.
        </p>
      </aside>
    </>
  );
